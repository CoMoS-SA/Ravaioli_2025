...
void CLIMATE_POLICY(void)
{
    ...
    if(flag_tax_CO2==5 && t > t_regime_shifts) //as flag == 1 but introduced during the simulation
    {
        //Constant tax increasing with inflation
        t_CO2=cpi(2)/cpi_t_regime_shifts*t_CO2_0; 
        t_CO2_en=cpi(2)/cpi_t_regime_shifts*t_CO2_en_0;
    }


    if(flag_tax_CO2==6 && t > t_regime_shifts) //as flag == 3 but introduced during the simulation
    {
        //Constant tax increasing with inflation
        t_CO2=cpi(2)/cpi_t_regime_shifts*(t_CO2_0*pow(tc2,(t-t_regime_shifts)));
        t_CO2_en=cpi(2)/cpi_t_regime_shifts*(t_CO2_en_0*pow(tc2,(t-t_regime_shifts)));
    }

    //Ensure that tax rate is not negative
    t_CO2=max(t_CO2,0.0);  
...
{  
    ...
    for(i=1; i<=nshocks; i++)
    {
        X_a(i)=a_0(i)*pow(1+log((Tmixed(2))/(Tmixedinit1)),shockexponent1(i));
        X_b(i)=b_0(i)*pow((Tmixedinit1)/(Tmixed(2)),shockexponent2(i));
    }
    ...
    if (t>(t_start_climbox+4))
    {
        ...
        if (flag_prodshocks1==7)
        {
            for (i=1; i<=N1; i++)
            {
                rnd=betadev(X_a(1),X_b(1),p_seed);
                shocks_machprod(i)=max(0.0,rnd-shocks_machprod(i));
                A1(i)=max(A0,A1(i)*(1-shocks_machprod(i)));

                rnd=betadev(X_a(1),X_b(1),p_seed);
                shocks_techprod(i)=max(0.0,rnd-shocks_techprod(i));
                A1p(i)=max(A0*pm,A1p(i)*(1-shocks_techprod(i)));
            } 
        }

    
        if (flag_prodshocks1==8)
        {
            for (i=1; i<=N1; i++)
            {
                rnd=betadev(X_a(1),X_b(1),p_seed);
        ...
        if (flag_prodshocks1==11)
        {
            for (i=1; i<=N1; i++)
            {
                rnd=betadev(X_a(1),X_b(1),p_seed);
                if(rnd>shocks_machprod(i))
                {
                    shocks_machprod(i)=rnd-shocks_machprod(i);
                    A1_en(i)=max(A0_en,A1_en(i)*(1-shocks_machprod(i)));
                    for (tt=t0; tt<=t; tt++)
                    {
                        A_en(tt,i)=max(A0_en,A_en(tt,i)*(1-shocks_machprod(i)));
                ...
                rnd=betadev(X_a(1),X_b(1),p_seed); 
                if(rnd>shocks_techprod(i))
                {
                    shocks_techprod(i)=rnd-shocks_techprod(i);
                    A1p_en(i)=max(A0_en,A1p_en(i)*(1-shocks_techprod(i)));
                }
            }    
        }

        if (flag_prodshocks1==12)
        {
            for (i=1; i<=N1; i++)
            {
                rnd=betadev(X_a(1),X_b(1),p_seed);
...
void SHOCKSNORD(void)
{  
    //This function generates repeated endogenous climate shocks following a DICE-type damage function
    if (t>(t_start_climbox+4))
    {
        shock_nord=a2_nord*pow(Tmixed(2),2);
        if(flag_capshocks>0 && flag_outputshocks>0)
        {
            shock_nord=0.5*shock_nord;
        }
        ...
        if (flag_prodshocks1==1)
        {
            for (i=1; i<=N1; i++)
            {
                rnd=max(0.0,shock_nord+shock_nord/sd_nord*gasdev(p_seed));
                shocks_machprod(i)=rnd;
                A1(i)=max(A0,A1(i)*(1-shocks_machprod(i)));
        ...
        if (flag_prodshocks1==7)
        {
            for (i=1; i<=N1; i++)
            {
                ...
                {   
                    rnd=max(0.0,shock_nord);
                }
                if(rnd>shocks_machprod_max(i))
                {
                    shocks_machprod(i)=rnd-shocks_machprod_max(i);
                    shocks_machprod_max(i)=rnd;
                }
                else
                {
                    shocks_machprod(i)=0;
                ...
                if(rnd>shocks_techprod_max(i))
                {
                    shocks_techprod(i)=rnd-shocks_techprod_max(i);
                    shocks_techprod_max(i)=rnd;
                }
                else
                {
                    shocks_techprod(i)=0;
        ...
        if (flag_encapshocks>0)
        {
            ...
            for (tt=1; tt<=t; tt++)
            {
                shocks_encapstock_de(t)=shock_nord;
                if (G_de(tt)>0)
                {
                    G_de(tt)=ROUND(G_de(tt)*(1-shocks_encapstock_de(t)));
                }
                
                shocks_encapstock_ge(t)=shock_nord;
                if (G_ge(tt)>0)
                {
                    G_ge(tt)=ROUND(G_ge(tt)*(1-shocks_encapstock_ge(t)));
                    G_ge_n(tt)=ROUND(G_ge_n(tt)*(1-shocks_encapstock_ge(t)));
